...
from .epcl_transformer import EPCLEncoder, EPCLPreEncoder
...
class EPCLDetection(nn.Module):
    ...
    def __init__(
        ...
        encoder_dim=256,
        voxel_size=None,
        in_channels=7  # Modified from 512 to match actual input dimension
    ):
        super().__init__()
        self.voxel_size = voxel_size
        
        # Feature projection layer to handle dimensional adaptation from 7→512
        self.feature_projection = nn.Sequential(
            ME.MinkowskiConvolution(
                in_channels, 512, kernel_size=1, stride=1, dimension=3),
            ME.MinkowskiBatchNorm(512, momentum=0.1),
            ME.MinkowskiReLU(inplace=True),
        )
        
        # Modified to accept projected features (512 channels)
        self.embdding_layer = nn.Sequential(
            ME.MinkowskiConvolution(
                512, 64, kernel_size=1, stride=1, dimension=3),
        ...
        self.out_layer = nn.Sequential(
            ME.MinkowskiConvolution(128, 256, kernel_size=1, bias=False, dimension=3),
            ME.MinkowskiBatchNorm(256, momentum=0.1),
            ME.MinkowskiReLU(inplace=True),
        )
...
def build_preencoder(args):
    # Updated to use correct input channel dimension
    preencoder = EPCLPreEncoder(
        num_group=args.PREENC_NPOINTS,
        group_size=args.GROUPS_SIZE,  
        enc_dim=args.ENC_DIM,
        in_channels=args.PREENCODER_DIM  # Now correctly set to 7
    )
...
def build_epcl(args):
    ...
    model = EPCLDetection(
                ...
                in_channels=7  # Modified to reflect actual input features
            )
