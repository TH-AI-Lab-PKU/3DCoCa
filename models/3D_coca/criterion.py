...
class VoteQueryCriterion(nn.Module):
    ...
    def forward(self, outputs: Dict, targets: Dict) -> Dict:
        ...
        # ==== auxiliary loss
        if 'revote_vote_xyz' in outputs:
            
            for vote_xyz in outputs['revote_vote_xyz']:
                
                for loss_name, (loss_fn, loss_weight) in self.loss_dict.items():
                    
                    temp = {
                        'vote_xyz': vote_xyz,
                        'seed_xyz': outputs['revote_seed_xyz'],
                        'seed_inds': outputs['revote_seed_inds'],
                    }
                    loss_intermidiate = loss_fn(temp, targets)
                    loss_dict.update(loss_intermidiate)
