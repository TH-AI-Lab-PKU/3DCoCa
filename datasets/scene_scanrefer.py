...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split in ['train','val']:
            # Collect unique instance IDs in the scene
            unique_instance_ids = np.unique(instance_bboxes[:, -1])
            ...
            if scan_name in self.gathered_language:
                ...
                for instance_id in unique_instance_ids:
                    if instance_id in self.gathered_language[scan_name]:
                        instance_caption = random.choice(self.gathered_language[scan_name][instance_id])
                    else:
                        instance_caption = ''
                    if int(instance_id) in self.dataset_config.class2type:
                        instance_caption_r += 'The description of instance '+self.dataset_config.class2type[int(instance_id)]+' is:'+ instance_caption
                    else:
                        instance_caption_r += 'The description of instance '+str(int(instance_id))+' is:'+ instance_caption
                # Compute category counts for the scene based on instance_bboxes
                target_categories = [
                    self.dataset_config.class2type[self.dataset_config.nyu40id2class[int(x)]] 
                    for x in instance_bboxes[:, -2][:instance_bboxes.shape[0]]]
                category_counts = dict(Counter(target_categories))
                category_summary = f"There are {len(category_counts)} categories. The distribution is as follows: " + "; ".join([f"{key}: {value}" for key, value in category_counts.items()])

                # Prepend summary information
                caption = f"sos There are {total_instances} instances in {scan_name}. " + category_summary + " " + instance_caption_r + " eos"
                scene_caption.append(caption)
            else:
                scene_caption = ['']
            
            if self.args.vocabulary == 'clib':
                tokenizer_output = self.tokenizer.encode(scene_caption[0])
                tokenizer_np = np.array(tokenizer_output)
                # tokenizer_np = tokenizer_np.squeeze(0)
                fixed_length = 500
                if tokenizer_np.shape[0] < fixed_length:
                    pad_length = fixed_length - tokenizer_np.shape[0]
                    tokenizer_np = np.concatenate([tokenizer_np, np.zeros((pad_length,), dtype=tokenizer_np.dtype)])
                elif tokenizer_np.shape[0] > fixed_length:
                    tokenizer_np = tokenizer_np[:fixed_length]
                attn_mask = (tokenizer_np != 0).astype(np.float32)
            else:
                tokenizer_output = self.tokenizer.batch_encode_plus(
                    scene_caption, 
                    max_length=self.max_des_len, 
            ...
            ret_dict['reference_tokens'] = tokenizer_np.astype(np.int64)
            ret_dict['reference_masks'] = attn_mask.astype(np.float32)
            ret_dict['scene_caption'] = scene_caption[0]
